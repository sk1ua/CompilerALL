...
enum enum_Opt {LOAD, LOADI, STO, STI, ADD, SUB,
                    ...
                    LES, GE, LE, AND, OR, NOT, IN, OUT, RETURN,ENTER,CAL,LOADA, STOA};
...
int TESTmachine()
{
    ...
    while (is >> codes[numCodes].opt >> codes[numCodes].operand) {
        // cout << "Read " << codes[numCodes].opt << " " << codes[numCodes].operand << endl;
        ++numCodes;
    }
    ...
    do{ //执行每条指令
        ...
        // cout << "Execute " << instruction.opt << " " << instruction.operand << endl;
        // cout << "start frame: ";
        ...
        switch(Map[instruction.opt]){
            ...
            case CAL:
                ...
                break;
            case LOADA:
                stack[top - 1] = stack[base + stack[top - 1]];
                break;
            case STOA:
                top -= 2;
                stack[base + stack[top + 1]] = stack[top]; 
            }
            // cout << "  end frame: ";
            // for (int i = base; i < top; ++i) {
            //     cout << stack[i] << ' ';
